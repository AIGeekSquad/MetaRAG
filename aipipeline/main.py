...
def process_data(directory: str, collection_name: str, config_type: str=None, config: IngestionConfigurationParm=None, **datargs):
    """
    Processes data from a specified directory or from Wikipedia topics and writes embeddings.
    Args:
        directory (str): The directory containing data files to be processed.
        ...
        config (IngestionConfigurationParm, optional): Configuration parameters for data ingestion. Defaults to None.
        **datargs: Additional arguments for data processing, such as 'topics' and 'page_count_per_topic' for Wikipedia data.
    Returns:
        None
    """
    ...
    if datargs.get("topics") is not None:
        ...
        write_embedding(nodes, collection_name=collection_name, embed_model=embeddings_model, llm=llm_model)
...
validate_vdb_collection(collection_name = vdb_collection_name)
